    ...
    final r = await showDialog<bool>(context: context, builder: (d) => StatefulBuilder(
      builder: (context, setDialogState) => AlertDialog(
        ...
        content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
          DropdownButtonFormField<String>(initialValue: type, decoration: InputDecoration(prefixIcon: icon3D(Icons.receipt, size: 22), labelText: 'Finance Type'), items: const [DropdownMenuItem(value: 'Salary Received', child: Text('Salary Received')), DropdownMenuItem(value: 'Advance Received', child: Text('Advance Received')), DropdownMenuItem(value: 'Other', child: Text('Other'))], onChanged: (v) { if (v != null) setDialogState(() => type = v); }),
        ...
        actions: [TextButton(onPressed: () => Navigator.pop(d, false), child: const Text('Cancel')), ElevatedButton.icon(onPressed: () async { final amount = double.tryParse(ac.text) ?? 0; if (amount <= 0) return; await DatabaseHelper.instance.updateFinanceRecord(FinanceRecord(id: record.id, type: type, amount: amount, reason: rc.text, recordDate: record.recordDate, createdTime: record.createdTime)); if (!d.mounted) return; Navigator.pop(d, true); }, icon: icon3D(Icons.save, size: 20), label: const Text('Update'))],
      ),
    ));
    ac.dispose(); rc.dispose();
    if (r == true) { await loadFinance(); if (!mounted) return; ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Finance Record Updated ✅'))); }
  }
  ...
  Future<void> addFinanceRecord() async {
    ...
    final r = await showDialog<bool>(context: context, builder: (d) => StatefulBuilder(
      builder: (context, setDialogState) => AlertDialog(
        ...
        actions: [TextButton(onPressed: () => Navigator.pop(d, false), child: const Text('Cancel')), ElevatedButton.icon(onPressed: () async { final amount = double.tryParse(ac.text) ?? 0; if (amount <= 0) return; await DatabaseHelper.instance.insertFinanceRecord(FinanceRecord(type: type, amount: amount, reason: rc.text, recordDate: DateFormat('dd-MM-yyyy').format(DateTime.now()), createdTime: DateFormat('hh:mm a').format(DateTime.now()))); if (!d.mounted) return; Navigator.pop(d, true); }, icon: icon3D(Icons.save, size: 20), label: const Text('Save'))],
      ),
    ));
    ac.dispose(); rc.dispose();
    if (r == true) { await loadFinance(); if (!mounted) return; ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Finance Record Saved ✅'))); }
  }
