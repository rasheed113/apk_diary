...
class _HistoryPageState extends State<HistoryPage> {
  ...
  List<DateTime> _weeksForLastFourMonths() {
    ...
    final c = DateTime(n.year, n.month - 3, 1);
    var x = _startOfWeek(n);
    final r = <DateTime>[];
    while (!x.isBefore(c)) {
      r.add(x);
      x = x.subtract(const Duration(days: 7));
    }
    return r;
  }

  List<DateTime> _recentMonths(int c) {
    final n = DateTime.now();
    return List.generate(c, (i) => DateTime(n.year, n.month - i, 1));
  }
    ...
    return Expanded(
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: s.primary.withValues(alpha: .16)),
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [s.primary.withValues(alpha: .14), s.secondary.withValues(alpha: .08)]),
                  border: Border.all(color: s.primary.withValues(alpha: .14)),
                  boxShadow: [BoxShadow(color: s.primary.withValues(alpha: .08), blurRadius: 6, offset: const Offset(0, 3))],
                ),
                child: Icon(icon, size: 19, color: s.primary),
              ),
              const SizedBox(width: 8),
              Flexible(child: Text(label, overflow: TextOverflow.ellipsis, style: TextStyle(fontWeight: FontWeight.w900, color: s.primary))),
            ]),
          ),
        ),
      ),
  ...
  Future<void> _showWeeklyHistory() async {
    ...
    await showModalBottomSheet<void>(context: context, isScrollControlled: true, showDragHandle: true, builder: (sc) {
      ...
      return SafeArea(child: SizedBox(height: MediaQuery.of(sc).size.height * .78, child: Column(children: [
        Padding(padding: const EdgeInsets.fromLTRB(20, 4, 20, 12), child: Text(l.weeklyHistory, style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800, color: s.onSurface))),
        Expanded(child: ListView.builder(itemCount: weeks.length, itemBuilder: (c, i) {
          final w = weeks[i];
          return ListTile(leading: Icon(Icons.calendar_view_week, color: s.primary), title: Text(i == 0 ? l.thisWeek : '$i ${l.week}', style: TextStyle(fontWeight: FontWeight.w700, color: s.onSurface)), subtitle: Text(_weekLabel(w), style: TextStyle(color: s.onSurfaceVariant)), trailing: Icon(Icons.chevron_right, color: s.primary), onTap: () { Navigator.pop(sc); setState(() => selectedFilter = 'Week:${w.toIso8601String()}'); });
        })),
      ])));
    });
  }
  ...
  Future<void> _showMonthlyHistory() async {
    ...
    await showModalBottomSheet<void>(context: context, isScrollControlled: true, showDragHandle: true, builder: (sc) {
      ...
      final l = _l;
      return SafeArea(child: SizedBox(height: MediaQuery.of(sc).size.height * .78, child: Column(children: [
        Padding(padding: const EdgeInsets.fromLTRB(20, 4, 20, 12), child: Text(l.monthlyHistory, style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800, color: s.onSurface))),
        Expanded(child: ListView.builder(itemCount: months.length, itemBuilder: (c, i) {
          final m = months[i];
          return ListTile(leading: Icon(Icons.calendar_month, color: s.primary), title: Text(i == 0 ? l.thisMonth : '$i ${l.month}', style: TextStyle(fontWeight: FontWeight.w700, color: s.onSurface)), subtitle: Text(_monthLabel(m), style: TextStyle(color: s.onSurfaceVariant)), trailing: Icon(Icons.chevron_right, color: s.primary), onTap: () { Navigator.pop(sc); setState(() => selectedFilter = 'Month:${m.year}-${m.month.toString().padLeft(2, '0')}'); });
        })),
      ])));
    });
  }
  ...
  Future<void> _searchByMonth() async {
    final n = DateTime.now();
    final l = _l;
    final p = await showDatePicker(context: context, initialDate: n, firstDate: DateTime(2000), lastDate: DateTime(n.year + 1, 12, 31), helpText: l.selectAnyDateInMonth, fieldLabelText: l.monthLabel);
  ...
  Future<void> deleteEntry(int id) async {
    ...
    if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(_l.entryDeletedSuccessfully)));
  }

  Future<void> confirmDelete(DiaryEntry entry) async {
    final l = _l;
    final confirm = await showDialog<bool>(context: context, builder: (c) => AlertDialog(title: Text(l.deleteEntry), content: Text('${l.deleteEntryConfirmation}\n\n${entry.itemName}'), actions: [TextButton(onPressed: () => Navigator.pop(c, false), child: Text(l.cancel)), TextButton(onPressed: () => Navigator.pop(c, true), child: Text(l.delete))]));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(backgroundColor: s.surface, appBar: AppBar(title: Text(l.history), backgroundColor: s.surface, elevation: 0), body: FutureBuilder<List<DiaryEntry>>(future: entriesFuture, builder: (context, snapshot) {
      ...
      final filtered = entries.where((entry) {
        ...
        final d = _parseDate(entry.workDate);
        ...
        Expanded(child: filtered.isEmpty ? Center(child: Text(l.noEntriesYet, style: TextStyle(color: s.onSurfaceVariant))) : ListView.builder(itemCount: filtered.length, itemBuilder: (context, index) {
          ...
          return themedCard(
            ...
            child: ListTile(
              ...
              leading: Icon(Icons.receipt_long_rounded, color: s.primary, size: 25),
              ...
              subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text('${l.total}: Rs ${entry.total.toStringAsFixed(0)}', style: TextStyle(color: s.onSurfaceVariant, fontWeight: FontWeight.w700)),
                Text('$dayName, ${entry.workDate}', style: TextStyle(color: s.onSurfaceVariant)),
              ]),
              trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                IconButton(padding: EdgeInsets.zero, constraints: const BoxConstraints(minWidth: 38, minHeight: 38), tooltip: l.edit, icon: Icon(Icons.edit_rounded, size: 21, color: s.primary), onPressed: () async { final r = await Navigator.push(context, MaterialPageRoute(builder: (_) => WorkPage(entry: entry))); if (r == true && mounted) setState(refreshEntries); }),
                IconButton(padding: EdgeInsets.zero, constraints: const BoxConstraints(minWidth: 38, minHeight: 38), tooltip: l.delete, icon: Icon(Icons.delete_outline_rounded, size: 23, color: s.error), onPressed: () => confirmDelete(entry)),
              ]),
            ),
          );
        })),
