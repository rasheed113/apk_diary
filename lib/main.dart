...
class _MyAppState extends State<MyApp> {
  @override Widget build(BuildContext context) => ValueListenableBuilder<AppTheme>(
    valueListenable: AppThemeController.currentTheme,
    builder: (context, theme, child) => MaterialApp(debugShowCheckedModeBanner: false, title: 'WorkEarn', theme: ThemeManager.getTheme(theme), home: const SplashScreen()),
  );
}
...
class _SplashScreenState extends State<SplashScreen> with SingleTickerProviderStateMixin {
  ...
  @override void initState() {
    ...
    _controller.forward();
    Timer(const Duration(milliseconds: 2300), () { if (mounted) Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (_) => const HomePage())); });
  }
    ...
    return Scaffold(
      body: Container(
        ...
        child: Center(child: FadeTransition(opacity: _fade, child: ScaleTransition(scale: _scale, child: Column(mainAxisSize: MainAxisSize.min, children: [
          // The current PNG contains a white matte. Multiply lets that white matte visually disappear on the bright surface.
          ColorFiltered(
            colorFilter: const ColorFilter.mode(Colors.white, BlendMode.multiply),
            child: Image.asset('assets/branding/workearn_logo.png', width: 170, height: 170, fit: BoxFit.contain),
          ),
          const SizedBox(height: 22),
          Text('WORK EARN', style: TextStyle(color: s.primary, fontSize: 25, fontWeight: FontWeight.w900, letterSpacing: 2.2, shadows: [Shadow(color: s.primary.withValues(alpha: .28), blurRadius: 5, offset: const Offset(1, 2)), const Shadow(color: Colors.white70, blurRadius: 2, offset: Offset(-1, -1))])),
...
class _HomePageState extends State<HomePage> {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(body: pages[selectedIndex], bottomNavigationBar: Container(
      ...
      child: ClipRRect(borderRadius: BorderRadius.circular(28), child: NavigationBar(height: 72, elevation: 0, backgroundColor: t.cardColor, indicatorColor: t.colorScheme.primary.withValues(alpha: .14), labelBehavior: NavigationDestinationLabelBehavior.alwaysShow, selectedIndex: selectedIndex,
        destinations: [NavigationDestination(icon: nav3D(Icons.dashboard), label: 'Dashboard'), NavigationDestination(icon: nav3D(Icons.work), label: 'Work'), NavigationDestination(icon: nav3D(Icons.history), label: 'History'), NavigationDestination(icon: nav3D(Icons.account_balance_wallet), label: 'Finance'), NavigationDestination(icon: nav3D(Icons.settings), label: 'Settings')],
